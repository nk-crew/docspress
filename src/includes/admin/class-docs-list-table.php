...
class DocsPress_Docs_List_Table {
    ...
    function add_meta_box() {
        add_meta_box( 'op-menu-meta-box-id', __( 'Helpfulness', DOCSPRESS_DOMAIN ), array( $this, 'helpfulness_metabox' ), 'docs', 'side', 'core' );
    }
    ...
    public function docs_list_columns( $columns ) {
        $vote  = array( 'votes' => __( 'Votes', DOCSPRESS_DOMAIN ) );
    ...
    public function docs_list_columns_row( $column_name, $post_id ) {
        if ( 'votes' == $column_name ) {
            ...
            printf( '<span class="docspress-positive">%d</span>/<span class="docspress-negative">%d</span>', $positive, $negative );
        }
    }
